...
let userID = "bc192337-d8ba-48e7-b911-e88ae2dc66fb";
            ...
            if (!upgradeHeader || upgradeHeader !== "websocket") {
                ...
                switch (url.pathname) {
                    ...
                    case `/sub/${userID}`:
                        ...
                        const vlessConfigs = client === "singbox" ? await env.Reza.get("singbox-sub") : await env.Reza.get("xray-sub");
                    ...
                    case `/frag/${userID}`:
                        ...
                        const configs = JSON.parse(await env.Reza.get("fragConfigs"));                        
                    ...
                    case `/panel`:
                        ...
                        const hostValue = await env.Reza.get("host");                        
                        if (!hostValue) {
                            await updateDataset(
                            env,
                            host, 
                        ...
                        if (hostValue !== host) await env.Reza.put("host", host);
                        
                        if (request.method === "POST" || !await env.Reza.get("fragConfigs") || hostValue !== host) {
...
const getVLESSConfig = async (env, userID) => {
    let vlessWsTls = "";
    const cleanIPs = await env.Reza.get("cleanIPs");
    const hostName = await env.Reza.get("host");
    ...
    await env.Reza.put("xray-sub", xraySub);
    await env.Reza.put("singbox-sub", singboxSub);
};
...
const getFragVLESSConfig = async (env, userID) => {
    ...
    Addresses.forEach((addr, index) => {
        ...
        fragConfig.remarks = `Reza Frag - ${addr}`;
    ...
    bestPingConfig.remarks = 'Reza Frag - Best Ping 💥';
    ...
    await env.Reza.put("fragConfigs", JSON.stringify(Configs));
};
...
const renderPage = async (env, uuid) => {
    ...
    const pwd = await env.Reza.get("Reza");
	...
	<body>
		<h1 style="text-align: center; color: #2980b9">Reza Panel <span style="font-size: smaller;">2.2</span> 💦</h1>
		<div class="form-container">
			...
			<div class="table-container">
				<table id="normal-configs-table">
					...
					<tr>
						...
						<td>
                            <button onclick="copyToClipboard('https://${host}/sub/${uuid}#Reza%20Normal', false)">
                                Copy 
					...
					<tr>
						...
						<td>
                            <button onclick="copyToClipboard('https://${host}/sub/${uuid}?app=singbox#Reza-Normal', false)">
                                Copy 
                                <span class="material-symbols-outlined">format_list_bulleted</span>
                            </button>
						</td>
			...
			<div class="table-container">
                <table id="normal-configs-table">
                    ...
                    <tr>
                        ...
                        <td>
                            <button onclick="copyToClipboard('https://${host}/fragsub/${uuid}#Reza Fragment', false)">
                                Copy Sub
                                <span class="material-symbols-outlined">format_list_bulleted</span>
                            </button>
                        </td>
                    ...
                    <tr>
                        ...
                        <td>
                            <button onclick="copyToClipboard('https://${host}/frag/${uuid}?addr=Best-Ping#Reza Fragment Best Ping', false)">
                                Copy Sub
                                <span class="material-symbols-outlined">format_list_bulleted</span>
                            </button>
                        </td>
            ...
            <div class="footer">
                ...
                <a class="link" href="https://github.com/bia-pain-bache/Reza-Worker-Panel" target="_blank">
                    Github
                </a>
...
const renderLoginPage = async () => {
    ...
    <body>
        <div class="container">
            <h1>Reza Panel <span style="font-size: smaller;">2.2</span> 💦</h1>
            <div class="form-container">
                <h2>User Login</h2>
                <form id="loginForm">
                    <div class="form-control">
...
const updateDataset = async (env, host, remoteDNS, localDNS, lengthMin, lengthMax, intervalMin, intervalMax, cleanIPs) => {
    ...
    for (const [key, value] of Object.entries(initData)) {
        await env.Reza.put(key, value);
    }
};

const getDataset = async (env) => {
    const host = await env.Reza.get("host");
    const remoteDNS = await env.Reza.get("remoteDNS");
    const localDNS = await env.Reza.get("localDNS");
    const lengthMin = await env.Reza.get("lengthMin");
    const lengthMax = await env.Reza.get("lengthMax");
    const intervalMin = await env.Reza.get("intervalMin");
    const intervalMax = await env.Reza.get("intervalMax");
    const cleanIPs = await env.Reza.get("cleanIPs");
    const fragConfigs = JSON.parse(await env.Reza.get("fragConfigs"));
